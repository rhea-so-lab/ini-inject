...
export function LoadFile(fileName: string): ClassDecorator {
	return (object: any) => {
		const parsedData = ini.parse(fs.readFileSync(path.join(process.cwd(), `${fileName}.ini`), { encoding: 'utf-8' }));
		...
		if (columns) {
			...
			for (const column of columns) {
				if (object[column.name] === undefined) {
					throw new Error(`${column.name} is not defined in ${fileName}.ini`);
				}
			}
		}
	}
